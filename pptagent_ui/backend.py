...
@app.websocket("/wsapi/{task_id:path}")
...
async def ppt_gen(task_id: str, rerun=False):
    ...
    try:
        ...
        # 临时修改部分代码以兼容旧版本的 image_stats.json（没有 caption 字段），并确保每张图片都有 caption 字段，避免后续处理报错
        # labler = ImageLabler(presentation, pptx_config)
        # image_stats_path = pjoin(pptx_config.RUN_DIR, "image_stats.json")

        # if os.path.exists(image_stats_path):
        #     with open(image_stats_path, "r", encoding="utf-8") as f:
        #         image_stats = json.load(f)

        #     # 兼容旧 image_stats.json：没有 caption 的一律补空字符串
        #     if not isinstance(image_stats, dict):
        #         image_stats = {}

        #     for image in image_stats:
        #         if not isinstance(image_stats[image], dict):
        #             image_stats[image] = {}
        #         if "caption" not in image_stats[image]:
        #             image_stats[image]["caption"] = ""

        #     labler.apply_stats(image_stats)

        #     # 把修补后的 image_stats 写回去，避免下次继续报 KeyError: 'caption'
        ...
        # else:
        #     # 为幻灯片中的图像生成标题；如果当前是纯文本模型，则跳过并补空 caption
        #     try:
        #         await labler.caption_images_async(models.vision_model)
        #     except Exception as e:
        #         print(f"[WARNING] Skip image captioning due to vision model/API issue: {e}")

        #     # 无论 caption 是否成功，都保证每张图片都有 caption 字段
        #     if not isinstance(labler.image_stats, dict):
        #         labler.image_stats = {}

        #     for image in labler.image_stats:
        #         if not isinstance(labler.image_stats[image], dict):
        #             labler.image_stats[image] = {}
        #         if "caption" not in labler.image_stats[image]:
        #             labler.image_stats[image]["caption"] = ""
